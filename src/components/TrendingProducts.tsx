...
export function TrendingProducts({ maxDisplay = 4 }: TrendingProductsProps) {
    ...
    useEffect(() => {
        const fetchTrending = async () => {
            setLoading(true);
            try {
                const [trendingProducts, cats] = await Promise.all([
                    productsApi.getTrending(maxDisplay),
                ...
                // Fetch images for trending products
                const imagePromises = trendingProducts.map(async (p) => {
                    try {
